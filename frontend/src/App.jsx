...
export default function App() {
  ...
  useEffect(() => {
    const h = e => {
      ...
      if (simRef.current && !simRef.current.contains(e.target))        setSimPanel(false)
    }
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', background: '#0d0f14', color: '#e6e9ef', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', padding: '0.3rem 1rem', borderBottom: '1px solid #2a3142', flexShrink: 0, position: 'relative' }}>
        ...
        <div ref={panelRef} style={{ position: 'relative' }}>
          <button onClick={() => { setConnPanel(v => !v); setDiscoverPanel(false); setAgentPanel(false); setSimPanel(false) }}
        ...
        <div ref={discoverRef} style={{ position: 'relative' }}>
          <button onClick={() => { setDiscoverPanel(v => !v); setConnPanel(false); setAgentPanel(false); setSimPanel(false) }}
        ...
        <div ref={agentRef} style={{ position: 'relative' }}>
          <button onClick={() => { setAgentPanel(v => !v); setConnPanel(false); setDiscoverPanel(false); setSimPanel(false) }}
        ...
        <div ref={simRef} style={{ position: 'relative' }}>
          <button onClick={() => { setSimPanel(v => !v); setConnPanel(false); setDiscoverPanel(false); setAgentPanel(false) }}
